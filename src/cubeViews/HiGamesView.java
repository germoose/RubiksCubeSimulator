...
import ca.germuth.rubiks.ScaledPath;
...
public class HiGamesView extends CubeView{
	...
	public void drawNumber(Canvas canvas, Paint paint){
		paint.setColor(this.colourToColor(Colour.GREEN));
		paint.setTextSize(18);
		canvas.drawText(this.inspection+"", 190, 460, paint);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_UP: 
			...
			float beginX = temp2X;
			if( endX >= beginX + 100){
				super.getMyCube().rotateLeft();
				this.invalidate();
			}
			if( endX <= beginX - 100){
				super.getMyCube().rotateRight();
				this.invalidate();
			}
			...
			float beginY = temp2Y;
			if( endY >= beginY + 100){
				super.getMyCube().rotateDown();
				this.invalidate();
			}
			if( endY <= beginY - 100){
				super.getMyCube().rotateUp();
				this.invalidate();
			}
    ...
    public void onDraw(Canvas canvas) {
		if( super.getMyCube() == null){
    		super.setMyCube(new Cube());
    	}
		if( super.getPaint() == null){
			super.setPaint( new Paint());
		}
    	
    	Cube myCube = super.getMyCube();
    	...
    	if( super.getMyCube().getIsSolved() && this.solving ){
